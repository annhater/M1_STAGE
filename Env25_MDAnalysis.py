...
def get_dynamic_environment(gro, xtc, cutoff=5.0):
    ...
    CA_asp_chB = u.select_atoms("name CA and resnum 50 and resname ILE and index 1536:3073")
...
for gro, xtc in sim_files:
    ...
    p = Path(src_path).parent.joinpath(f"Manips/Tables/{sim_name}_ILE_env_atoms.csv")
    ...
    p = Path(src_path).parent.joinpath(f"Manips/Tables/{sim_name}_ILE_env_atoms_frequency.csv")
