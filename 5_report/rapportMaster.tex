...
\begin{document}
	...
	\begin{abstract}
		...
		%{\small The HIV-2 protease ($\text{PR2}$) is a major therapeutic target whose conformational dynamics governs its catalytic activity. This study uses molecular dynamics simulations (500--1000~ns) to characterize the structural impact of the monoprotonated state ($\text{MP}$) versus the deprotonated state ($\text{DP}$) on the \textit{apo} enzyme. While $\text{MP}$ systematically stabilizes a ``classical'' closed conformation (flap A over flap B) with a rigid catalytic core, $\text{DP}$ induces stochastic behaviors, yielding either a permanently open state (V11) or an inverted closure (V1, flap B over A). Notably, the deprotonated replicate V12 spontaneously achieves the stable classical closed conformation. To identify the molecular drivers of this alternative stabilization, a multivariate classification was applied to hydrogen-bonding networks during the phase transition ($\pm 36$~ns). Jaccard distance clustering revealed trajectory-dependent interaction profiles, while a Decision Tree demonstrated three possible interactions controls the transition. The process is spearheaded by the local disruption of the $\text{ARG8\_NH1}-\text{GLY126\_O}$ contact between the upper loop and the active site, acting as the structural gatekeeper to seal the closed state. These physical rules provide valuable insights for designing specific inhibitors targeting $\text{PR2}$.}
	
		\par
		%{\small La protéase du VIH-2 ($\text{PR2}$) est une cible thérapeutique majeure dont la dynamique régit l'activité catalytique. Ce travail caractérise par dynamique moléculaire (500--1000~ns) l'impact de l'état de monoprotonation ($\text{MP}$) par rapport à l'état déprotoné ($\text{DP}$) sur la forme \textit{apo} de la $\text{PR2}$. Alors que la $\text{MP}$ stabilise systématiquement une conformation fermée «~classique~» (volet A devant B) et un noyau catalytique rigide, la $\text{DP}$ induit un comportement stochastique, menant à un état ouvert (V11) ou à une fermeture inversée (V1, volet B devant A). Étonnamment, le réplicat déprotoné V12 adopte spontanément la conformation fermée classique stable. Afin de décrypter les déterminants de cette stabilisation alternative, une classification multivariée a été appliquée aux réseaux de liaisons hydrogène lors de la transition inter-phasique ($\pm 36$~ns). Le partitionnement basé sur la distance de Jaccard montre des profils d'interactions dépendants de la trajectoire, tandis qu'un arbre de décision révèle qu'un ensemble minimal de trois interactions clés contrôle le passage de phase. Ce mécanisme est initié par la rupture locale du contact $\text{ARG8\_NH1}-\text{GLY126\_O}$ entre la boucle supérieure et le site catalytique, agissant comme le commutateur structural requis pour verrouiller la forme fermée. Ces règles physiques offrent de nouvelles pistes pour la conception rationnelle d'inhibiteurs spécifiques de la $\text{PR2}$.}
	\end{abstract}
	...
	\subsubsection{RMSF}
	Lorsqu'on a défini comment la structure évolue au cours de temps, on voudrait préciser quels sont les résidus qui sont impliqués dans le changement conformationnel.
	...
	La formule pour calculer les valeurs de RMSF, peut-être retrouvé en Annexes (\ref{A3}).
	
	\subsubsection{Distances inter-résidus}
	Par la suite, lorsqu'on a défini les résidus lesquels on voudrait observer plus en détail au cours de simulations, on a d'abord analysé les distances entre ces résidus d’intérêt.
	La matrice des distances (qui considère toutes les paires des atomes en fonction des frames) a été créé avec le logiciel GROMACS.
